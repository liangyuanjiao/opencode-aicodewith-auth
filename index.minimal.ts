/**
 * Step 6: Test with full fetch implementation
 */
...
console.log("[TEST] All imports completed")
...
export const AicodewithCodexAuthPlugin: Plugin = async (ctx: PluginInput) => {
  console.log("[TEST] Plugin function called - START")
  ...
  console.log("[TEST] ensureConfigFile completed")

  syncOmoConfig().catch((error) => {
    console.warn(`[${PACKAGE_NAME}] Failed to sync OMO config: ${error instanceof Error ? error.message : error}`)
  })
  console.log("[TEST] syncOmoConfig dispatched")

  const autoUpdateHook = createAutoUpdateHook(ctx, { autoUpdate: true })
  console.log("[TEST] autoUpdateHook created")

  console.log("[TEST] Creating authHook with full fetch implementation")
  const authHook: AuthHook = {
    provider: PROVIDER_ID,
    loader: async (getAuth: () => Promise<Auth>, _provider: Provider) => {
      console.log("[TEST] authHook.loader called")
      const auth = await getAuth()
      console.log("[TEST] auth retrieved, type:", auth.type)
      if (auth.type !== "api" || !auth.key) {
        console.log("[TEST] No API key, returning empty")
        return {}
      }
      ...
      console.log("[TEST] Returning full fetch handler")
      return {
        apiKey,
  ...
  console.log("[TEST] authHook created")

  console.log("[TEST] Returning hooks")
  return {
    auth: authHook,
    config: async (config) => {
...
console.log("[TEST] Plugin exported")
